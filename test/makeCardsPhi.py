...
def createLepHad(sig_hist, top_cr_hist, wlnu_cr_hist, qcd_cr_hist, sig_faildphi, top_cr_faildphi, wlnu_cr_faildphi, qcd_cr_faildphi, var_name, mttbins, ptbins, leptype, tmpdir, label, usingData, nnCut, nnCut_loose, metCut, lowMetCut, h_pt_min, singleBinCR, singleBinFail, lowqcdmass, lowqcdincrease, highbkgmass, highbkgincrease, highmassone, unifiedBkgEff, masspoints, shaperegion, year, doHtt, noSyst):
    ...
    sig_hists["lowmet_faildphi"] = sig_faildphi.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    top_cr_hists["lowmet_faildphi"] = top_cr_faildphi.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    wlnu_cr_hists["lowmet_faildphi"] = wlnu_cr_faildphi.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    qcd_cr_hists["lowmet_faildphi"] = qcd_cr_faildphi.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    ...
    sig_hists["lowmet"] = sig_hist.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    top_cr_hists["lowmet"] = top_cr_hist.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    wlnu_cr_hists["lowmet"] = wlnu_cr_hist.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    qcd_cr_hists["lowmet"] = qcd_cr_hist.group("process", hist.Cat("sample", "sample"), samp_combinations).integrate('met_pt',slice(lowMetCut,metCut),'none')
    ...
    lepid = rl.NuisanceParameter('CMS_id_had%s'%leptype, 'lnN')
    ...
    if year=='2017':
        lumi_list = [lumi_17, lumi_all, lumi_1718]
        lumi_vals = [1.02, 1.009, 1.006]
    elif year=='2018':
        lumi_list = [lumi_18, lumi_all, lumi_1718]
        lumi_vals = [1.015, 1.02, 1.002]
    else:
        lumi_list = [lumi_16, lumi_all]
        lumi_vals = [1.01, 1.006]
    ...
    syst_dict = {
        ...
        } for samp in samp_combinations if samp not in ['data_obs','ignore','multijet']
    }
    ...
    mttone = rl.Observable(var_name+'_one', np.array([mttbins[lowmassbin],mttbins[highmassbin-1]]))
    ...
    def getQCDfromData(inhist,region,default=1.,systematic='nominal',hptslice=slice(h_pt_min,None),mslice=None,unc_scale=0.):
        ...
        qcd_data_altdn = np.clip(qcd_data_altdn,default,None)
        qcd_data_altdn[qcd_data_altdn==default] = 0.
        qcd_data_altup = np.clip(qcd_data_altup,default,None)
        qcd_data_altup[qcd_data_altup==default] = 0.
...
#    qcdratio_wlnu = [((qcdfrom_wlnu["lowmet"][ptbin]/qcdfrom_qcd["lowmet"][ptbin]) + (qcdfrom_wlnu["faildphi"][ptbin]/qcdfrom_qcd["faildphi"][ptbin]))/2. for ptbin in range(npt)]
    ...
    for pfreg in ["fail","loosepass","pass"]:
        #for iq,qcdreg in enumerate(["faildphi","lowmet"]):
        for iq,qcdreg in enumerate(["lowmet","nom"]):
            for ptbin in range(npt):
        ...
        for iregion,region in enumerate(['fail','loosepass','pass']):
            ...
            for sName in thehist.identifiers('sample'):
                ...
                if singleBinFail and not isPass and not isLoosePass:
                    templ = (np.clip(np.array([np.sum(tempint[0][lowmassbin:highmassbin])]),0.,None), mttone.binning, mttone.name, np.array([np.sum(tempint[1][lowmassbin:highmassbin])]))
                else:
                    templ = (np.clip(tempint[0][lowmassbin:highmassbin],0.,None), mtt.binning, mtt.name, tempint[1][lowmassbin:highmassbin])
                ...
                if sName.name=='data_obs':
                    ...
                    if sName.name=='multijet':
                        qcdpred = np.clip(np.sum(np.stack([(qcd_from_data_sig["lowmet"][shaperegion[iregion]][ix][0]*qcdratio_sig[shaperegion[iregion]]["lowmet"][ptbin]*qcdratio_F["qcdnom"][region][ix]+qcd_from_data_qcd["nom"][shaperegion[iregion]][ix][0]*qcdratio_qcd[shaperegion[iregion]]["nom"][ptbin]*qcdratio_F["lowmet"][region][ix])/2. for ix in qcdbins[ptbin]] if len(qcdbins[ptbin])>0 else [np.zeros_like(qcd_from_data_sig["lowmet"][region][0][0])]),axis=0),0.,None)
                        qcdpred_dn = np.clip(np.sum(np.stack([qcd_from_data_sig["lowmet"][shaperegion[iregion]][ix][0] * qcdratio_sig[shaperegion[iregion]]["lowmet"][ptbin] * qcdratio_F["qcdnom"][region][ix] for ix in qcdbins[ptbin]] if len(qcdbins[ptbin])>0 else [np.zeros_like(qcd_from_data_sig["lowmet"][region][0][0])]),axis=0),0.,None)
                        qcdpred_up = np.sum(np.stack([qcd_from_data_qcd["nom"][shaperegion[iregion]][ix][0] * qcdratio_qcd[shaperegion[iregion]]["nom"][ptbin] * qcdratio_F["lowmet"][region][ix] for ix in qcdbins[ptbin]] if len(qcdbins[ptbin])>0 else [np.zeros_like(qcd_from_data_qcd["nom"][region][0][0])]),axis=0)
                        #qcdpred = np.clip(np.sum(np.stack([qcd_from_data_sig["lowmet"][shaperegion[iregion]][ix][0] * qcdratio_sig[shaperegion[iregion]]["lowmet"][ptbin] * qcdratio_F["qcdnom"][region][ix] for ix in qcdbins[ptbin]] if len(qcdbins[ptbin])>0 else [np.zeros_like(qcd_from_data_sig["lowmet"][region][0][0])]),axis=0),0.,None)
                        #qcdpred_dn = np.clip(np.sum(np.stack([qcd_from_data_sig["lowmet"][shaperegion[iregion]][ix][1] * qcdratio_sig_dn[shaperegion[iregion]]["lowmet"][ptbin] * qcdratio_F["qcdnom"][region][ix] for ix in qcdbins[ptbin]] if len(qcdbins[ptbin])>0 else [np.zeros_like(qcd_from_data_sig["lowmet"][region][0][0])]),axis=0),0.,None)
                        #qcdpred_up = np.clip(np.sum(np.stack([qcd_from_data_sig["lowmet"][shaperegion[iregion]][ix][2] * qcdratio_sig_up[shaperegion[iregion]]["lowmet"][ptbin] * qcdratio_F["qcdnom"][region][ix] for ix in qcdbins[ptbin]] if len(qcdbins[ptbin])>0 else [np.zeros_like(qcd_from_data_sig["lowmet"][region][0][0])]),axis=0),0.,None)
                        ...
                        if singleBinFail and not isPass and not isLoosePass:
                            ...
                            templ = (np.clip(qcdpred,0.,None), mtt.binning, mtt.name, np.ones_like(qcdpred)*1000000.)
                    if doHtt:
        ...
        for sName in thehist.identifiers('sample'):
            ...
            if sName.name=='data_obs':
                ...
                if sName.name in syst_dict:
                    ...
                    for syst in syst_dict[sName.name]:
                        syst_params = syst_dict[sName.name][syst]
                        dnmod = np.clip(intRegion(thehist[sName],region,systematic=syst_params[1])[0][lowmassbin:highmassbin],0.,None)
...
def createHadHad(sig_hist, top_cr_hist, wlnu_cr_hist, qcd_cr_hist, sig_faildphi, top_cr_faildphi, wlnu_cr_faildphi, qcd_cr_faildphi, var_name, mttbins, ptbins, tmpdir, label, usingData, nnCut_met, nnCut_met_loose, metCut, lowMetCut, h_pt_min, singleBinCR, singleBinFail, lowqcdmass, lowqcdincrease, highbkgmass, highbkgincrease, highmassone, unifiedBkgEff, masspoints, shaperegion, year, doHtt, noSyst):
    ...
    def getQCDfromDataFullMass(inhist,region,default=1.,systematic='nominal',hptslice=slice(h_pt_min,None),mslice=None,unc_scale=0.,verbose=0):
        ...
        qcd_data = np.clip(qcd_data,default,None)
        if verbose>0:
            print('QCDPRED (',verbose,'): ',region)
            print('data',qcd_data_full)
            print('mc',other_mc[0][lowmassbin:highmassbin])
        qcd_data[qcd_data==default] = 0.
        qcd_data_altdn = np.clip(qcd_data_altdn,default,None)
        qcd_data_altdn[qcd_data_altdn==default] = 0.
        qcd_data_altup = np.clip(qcd_data_altup,default,None)
    ...
    for qcdregion in ["faildphi","qcdfaildphi","qcdnom"]:
        qcdfail_temp[qcdregion] = {}
        qcdfail_temp_dn[qcdregion] = {}
        qcdfail_temp_up[qcdregion] = {}
        for region in ['fail','loosepass','pass']:
            ...
            if sName.name=='data_obs':
                ...
                if sName.name in syst_dict:
                    ...
                    for syst in syst_dict[sName.name]:
                        syst_params = syst_dict[sName.name][syst]
                        dnmod = np.clip(intRegion(thehist[sName],region,systematic=syst_params[1])[0][lowmassbin:highmassbin],0.,None)
                        upmod = np.clip(intRegion(thehist[sName],region,systematic=syst_params[2])[0][lowmassbin:highmassbin],0.,None)
                        if singleBinCR or (singleBinFail and not isPass and not isLoosePass):
                            dnmod = np.array([np.sum(dnmod)])
                            nom = np.array([np.sum(nom_base)])
                            upmod = np.array([np.sum(upmod)])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lowMetCutHad',         dest='lowMetCutHad', default=50.,                  help="low met cut (had)",   type=float)
